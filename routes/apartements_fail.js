...
console.log(req.body);
		...
		var multipleUpload = new Promise(async(resolve,reject) => {
			var	upload_len = filePaths.len,
				upload_res = new Array();

			for(var i=0; i<=(upload_len + 1); i++){
				...
				await cloudinary.uploader.upload(filePath, (error,result) => {
					if(upload_res.length === upload_len){
						...
						//upload_res.push({ public_id: result.public_id, secure_url: result.secure_url});
					}else if(error){
